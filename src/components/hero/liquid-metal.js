...
// @ts-nocheck
/* ---------------------------------------------------------------------------
   Liquid metal — a five-pass WebGL2 dispersion renderer.

   Ported verbatim from the ThreeUI SylvaHero reference (inner-green-3d.html).
   The renderer was already written against a shimmed `document`/`window` so it
   could mount into an arbitrary host element; that boundary is kept intact and
   only the teardown path is new — React unmounts these, the reference page
   never did.

   Why it earns its place on THIS site: the shader is a spectral dispersion
   model of light raking across a pinched metal sheet. On a sheet-metal
   stamper's homepage the CTA is, literally, formed steel.
   --------------------------------------------------------------------------- */

export function mountLiquidMetal(host) {
    ...
    /* The reference page never unmounted these, so the original shim just
       forwarded to the real window. React does unmount them, so every
       forwarded listener is recorded and the rAF carries a kill flag. */
    const __bound = [];
    let __dead = false;
    let disposer = null;
    const window = {
...
const C = window.__C = {
  // Reference values were 1.28 / 1.95 and 0.94 / 1.30, tuned against a mid-grey
  // green ground. On this near-black blue page the same bloom reads as a lamp
  // sitting behind the pill rather than as heat coming off it, so both are
  // pulled in.
  glow:   host.dataset.liquidMetal === 'play' ? 1.05 : 1.42,      // outer-glow gain
  glowR:  host.dataset.liquidMetal === 'play' ? 0.86 : 1.08,      // outer-glow radius
...
if(!gl){
  /* The reference owned the whole document and could replace it with a
     notice. Here the button is one control among many, so a missing WebGL2
     just leaves the CSS plate and its label standing on their own — the
     control still works, it simply is not molten. */
...
const ro = new ResizeObserver(() => { needResize = true; }); ro.observe(stage);
...
/* localPt returns [x, y]. The reference destructured straight into ptr.x and
   ptr.y here — valid on its own, but a statement that BEGINS with a bracket is
   an ASI hazard: once the production minifier joins statements it re-parses as
   an index into whatever precedes it, and the build dies at parse time with
   "Invalid destructuring assignment target". Dev never sees it because nothing
   is minified there. Assigning through a temp is immune. */
btn.addEventListener('pointerenter', e => {
  if(e.pointerType !== 'mouse') return;
  // land the well where the cursor actually entered, not where it last was
  const ptEnter = localPt(e); ptr.x = ptEnter[0]; ptr.y = ptEnter[1];
...
window.addEventListener('pointermove', e => {
  ...
  const ptMove = localPt(e); ptr.x = ptMove[0]; ptr.y = ptMove[1];
}, {passive:true});

btn.addEventListener('pointerdown', e => {
  const ptDown = localPt(e); ptr.x = ptDown[0]; ptr.y = ptDown[1];
  on.press = true; sync();
...
/* teardown: stop the loop, drop the forwarded listeners, and hand the GPU
   back its buffers — a hero that remounts on route change would otherwise
   leak a whole renderer per visit */
disposer = function(){
  if(__dead) return;
  __dead = true;
  for(const [type, fn, opts] of __bound) hostWindow.removeEventListener(type, fn, opts);
  __bound.length = 0;
  if(ro) ro.disconnect();
...
window.__seek   = v => { clock = v; drawn = null; };
    ...
    return function dispose(){ if(disposer) disposer(); };
  }
